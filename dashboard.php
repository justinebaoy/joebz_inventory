...
");
$today_sales = $q_today->fetch_assoc()['total'];
...
");
$month_sales = $q_month->fetch_assoc()['total'];

// Total number of items
$q_items = $conn->query("SELECT COUNT(*) AS total FROM items");
$total_items = $q_items->fetch_assoc()['total'];

// Low stock items (stock <= 5)
$q_low = $conn->query("SELECT COUNT(*) AS total FROM items WHERE stock <= 5");
$low_stock = $q_low->fetch_assoc()['total'];
...
$q_trans = $conn->query("
    ...
    WHERE DATE(sale_date) = '$today'
");
$today_transactions = $q_trans->fetch_assoc()['total'];
...
for ($i = 6; $i >= 0; $i--) {
    ...
    ");
    $chart_labels[] = $label;
    $chart_data[]   = (float)$q_chart->fetch_assoc()['total'];
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
      ...
      <div class="bg-slate-900/95 rounded-2xl p-5 border border-slate-800 shadow-xl shadow-black/20">
        ...
        <p class="text-xs text-slate-400 mt-1">
          <?= $today_transactions ?> transaction(s) today
        </p>
      </div>
      ...
      <div class="bg-slate-900/95 rounded-2xl p-5 border border-slate-800 shadow-xl shadow-black/20">
        ...
        <p class="text-xs text-red-300 mt-1">Items with stock ≤ 5</p>
      </div>
              ...
              <?php while ($row = $q_recent->fetch_assoc()): ?>
                <tr class="hover:bg-slate-800 transition">
                  <td class="px-5 py-3 font-mono text-slate-300">
                    #<?= str_pad($row['sale_id'], 4, '0', STR_PAD_LEFT) ?>
                  </td>
                  <td class="px-5 py-3 text-slate-100">
                    <?= htmlspecialchars($row['first_name'] . ' ' . $row['last_name']) ?>
                  </td>
                  <td class="px-5 py-3 text-slate-400">
                    <?= date('M d, Y h:i A', strtotime($row['sale_date'])) ?>
                  </td>
                  <td class="px-5 py-3 text-right font-semibold text-slate-100">
